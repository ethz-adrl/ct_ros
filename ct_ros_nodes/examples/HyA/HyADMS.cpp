...
void visualizeTrajectory(ct::ros::RBDStatePublisher& publisher,
    const ct::core::StateVectorArray<2 * njoints, double>& x,
    double dt)
{
...
int main(int argc, char* argv[])
{
    try
    {
        ...
        x_initguess.resize(settings.N_ + 1,
            ct::optcon::DmsSolver<HyASystemD::STATE_DIM, HyASystemD::CONTROL_DIM>::state_vector_t::Zero());
        u_initguess.resize(settings.N_ + 1,
            ct::optcon::DmsSolver<HyASystemD::STATE_DIM, HyASystemD::CONTROL_DIM>::control_vector_t::Zero());
        ...
        {
            x_initguess[i] = x0D.toImplementation() +
                             (xFD.toImplementation() - x0D.toImplementation()) * ((double)i / (double)settings.N_);
        ...
        while (ros::ok())
        {
            ...
            if (!solved)
            {
                ...
                std::cout << "DMS double solved problem? " << solved << ". Used iterations: " << iterationsD
                          << ", time: " << elapsedTimeD << "s" << std::endl;
